...
class Model(object):
    ...
    def train(self, src_train, src_dev, tags, target_train, target_dev, target_test, src_batch_size, target_batch_size):
        best_score = -1e-4
        ...
        return self.evaluate(target_test, tags, target='target')

    def evaluate(self, my_test, tags, target='src'):
        saver = tf.train.Saver()
        tf_config = tf.ConfigProto()
        tf_config.gpu_options.allow_growth = True
        tf_config.gpu_options.per_process_gpu_memory_fraction = self.args.gpu_frac
        ...
        with tf.Session(config=tf_config) as sess:
            ...
            acc, p, r, f1 = self.run_evaluate(sess, my_test, tags, target=target)
